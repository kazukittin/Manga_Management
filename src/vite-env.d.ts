...
interface Window {
    api: {
        ...
        getSavedRoot: () => Promise<string[] | string | null>;
        setRoot: (paths: string[]) => Promise<void>;
        ...
        fetchMetadataByTitle: (title: string) => Promise<{ title?: string; author?: string; publisher?: string; tags: string[]; description?: string; cover?: string; category?: 'manga' | 'novel' | 'reference' | 'other' | 'uncategorized' } | null>;
        saveCover: (filePath: string, base64Data: string) => Promise<{ success: boolean; error?: string }>;
    }
}
